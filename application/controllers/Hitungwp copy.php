...
class Hitungwp extends CI_Controller {
    ...
    public function index()
    {
        ...
        foreach ($alternative as $key1 => $value1) {
            ...
            $data_alternative[$key1][] = $value1[0] ."<sup>". $criteria[0]['normalisasi'] . "</sup><br><small>" .$vector . "</small>";
            for ($i=1; $i < sizeof($value1); $i++) { 
                $vector *= pow($value1[$i], $criteria[$i]['normalisasi']);
                $data_alternative[$key1][] = $value1[$i] . "<sup>". $criteria[$i]['normalisasi'] ."</sup><br><small>" . pow($value1[$i], $criteria[$i]['normalisasi']) . "</small>";
            }
